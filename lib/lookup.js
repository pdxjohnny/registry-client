...
function doRequest(name, index, callback) {
    ...
    var proxy = false;
    // this._config has to have a no_proxy property or it will
    // be treated as if no_proxy = ''
    var shouldProxyOptions = {
      no_proxy: this._config.no_proxy
    };
    if (shouldProxy(requestUrl, shouldProxyOptions)) {
